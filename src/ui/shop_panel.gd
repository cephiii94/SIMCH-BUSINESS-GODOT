...
func _populate_racks_list() -> void:
	...
	for i in range(ShopManager.racks.size()):
		...
		var item_ref: ItemData = DatabaseManager.get_item(rack["item_id"]) as ItemData
		if not item_ref:
			continue
		...
		stock_bar.size_flags_vertical = SIZE_CENTER
		...
		price_hbox.theme_override_constants.separation = 16
		...
		lbl_price_info.theme_override_font_sizes.font_size = 13
		...
		price_slider.size_flags_vertical = SIZE_CENTER
		...
		actions_hbox.theme_override_constants.separation = 8
		actions_hbox.alignment = FLOW_END
